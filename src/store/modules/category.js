...
export default {
  ...
  mutations: {
    ...
    // 控制当前分类的二级类目的显示
    show(state, id) {
      // console.log("Vuex接受的数据", id);
      // 当传递进来的分类id和数据中的id想等时(表示鼠标移入的是他,添加open为true,通过类名显示二级分类)
      const currCategory = state.list.find((item) => item.id === id);
      currCategory.open = true; // 打开当前的二级分类
    },
    // 控制当前分类的二级类目的隐藏
    hide(state, id) {
      // 当传递进来的分类id和数据中的id想等时(表示鼠标移入的是他,添加open为false,通过类名隐藏二级分类)
      const currCategory = state.list.find((item) => item.id === id);
